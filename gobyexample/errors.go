...
func f(arg int) (int, error) {
	if arg == 42 {
		...
		// errors.New constructs a new basic error value with a given error message 
...
// We can wrap errors with higher-level errors to add contaxt. Simplest way to do this is with the %w verb in fmt.Errorf
...
// In Go it's idiomatic to communciatemerrors via an explicit separate return value
// This contrasts to Java, Ruby and the overloaded single result/error value used in C
...
func Errors() {
	...
	for _, i := range []int{7, 42} {
		// it's common to use an inline error check in if line 
		if r, e := f(i); e != nil {
